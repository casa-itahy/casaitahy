<?php
	...
	if (!empty($pag_id)) {
		...
		if($pag_id == 1) {
			...
			$retorno = query("SELECT titulo, title, metad, conteudo, imagem, status FROM paginas WHERE tipo = 5 AND id = 23 OR id = 24 or id = 25 or id = 26 or id = 27 or id = 29 ORDER BY id ASC");
			if (empty($retorno)) {
				$textos = false;
			} else {
				$textos = $retorno;
