...
if($pai){
    $sql = "SELECT MIN(c.posicao) as posicao FROM categorias_subcat c
    ...
    //DESCOBRE A POSICAO DA CATEGORIA PAI ANTES DA ATUAL
        while ($flag == false){
            $posicaoAnterior = $posicaoAtual - $i;
            $sql ="SELECT csn.cat_pai_id, csn.cat_filho_id, c.posicao, c.id
                        ...
                        WHERE c.posicao = '$posicaoAnterior' AND csn.cat_filho_id IS NULL AND csn.cat_neto_id IS NULL";
        ...
        if($primeiro<$posicaoAtual){
            ...
            while ($flag == false){
                ...
                $sql ="SELECT csn.cat_pai_id, csn.cat_filho_id, c.posicao, c.id
                        ...
                        WHERE c.posicao = '$posicaoAnterior' AND csn.cat_pai_id='$pai_id' AND csn.cat_neto_id IS NULL";
            ...
            if($primeiro<$posicaoAtual){
                ...
                while ($flag == false){
                    ...
                    $sql ="SELECT csn.cat_pai_id, csn.cat_filho_id, c.posicao, c.id
                            FROM categorias_subcat c
                            INNER JOIN cat_subcat_nivel csn ON(c.id=csn.cat_filho_id)
                            WHERE c.posicao = '$posicaoAnterior' AND csn.cat_pai_id='$pai_id' AND csn.cat_filho_id='$filho_id'";
...
if($flag){
    ...
    $sql1 = "UPDATE $tabela SET posicao=$posicaoAnterior WHERE id=$id";
