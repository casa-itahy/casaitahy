    ...
    <div class="row align-items-end">
        <div class="col-md-6">
            <div class="contato-esquerda">
                <?php if (!empty($imagem)) { ?>
                    <img src="<?php echo IMAGENS . 'paginas/' . $imagem; ?>" alt="<?php echo $titulo; ?>"
                        title="<?php echo $titulo; ?>" />
                <?php } ?>
                <div class="informacoes-contato">
                    <h3>Endereço</h3>
                    <?php if (!empty($config['endereco'])): ?>
                        <p class="endereco-contato"><i class="fas fa-map-marker-alt"></i>
                            <?php echo nl2br($config['endereco']); ?>
                        </p>
                    <?php endif; ?>
                    <h3>Contato</h3>
                    <?php if (!empty($config['fone1'])): ?>
                        <a href="tel:<?php echo $config['fone1']; ?>" target="_blank" class="telefone-rodape"><i
                                class="fas fa-phone"></i>
                            <?php echo $config['fone1'] ?>
                        </a>
                    <?php endif; ?>
                    <a class="email-rodape" href="mailto:<?php echo $config['emailC']; ?>"><i
                            class="fas fa-envelope"></i>
                        <?php echo $config['emailC']; ?>
                    </a>
                    <h3>Siga-nos</h3>
